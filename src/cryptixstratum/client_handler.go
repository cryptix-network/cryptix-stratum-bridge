...
func (c *clientListener) NewBlockAvailable(cytxApi *CryptixApi, soloMining bool) {
	c.clientLock.Lock()
	...
	for _, cl := range c.clients {
		...
		go func(client *gostratum.StratumContext) {
			...
			template, err := cytxApi.GetBlockTemplate(client)
			if err != nil {
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := cytxApi.cryptixd.GetBalancesByAddresses(addresses)
				if err != nil {
