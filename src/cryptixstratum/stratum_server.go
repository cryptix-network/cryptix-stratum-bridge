...
type BridgeConfig struct {
	...
	RPCServer       string        `yaml:"cryptixd_address"`
...
func ListenAndServe(cfg BridgeConfig) error {
	...
	cytxApi, err := NewCryptixAPI(cfg.RPCServer, blockWaitTime, logger)
	if err != nil {
		return err
	}
	...
	shareHandler := newShareHandler(cytxApi.cryptixd)
	...
	defer cancel()
	cytxApi.Start(ctx, func() {
		clientHandler.NewBlockAvailable(cytxApi, cfg.SoloMining)
	})
